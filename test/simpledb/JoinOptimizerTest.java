...
public class JoinOptimizerTest extends SimpleDbTestBase {
	 ...
	 */
	@Test public void estimateJoinCardinalityTest() throws ParsingException {
        TransactionId tid = new TransactionId();
        Parser p = new Parser();
	...
	@Test(timeout=60000) public void bigOrderJoinsTest() throws IOException, DbException, TransactionAbortedException, ParsingException {
		...
		HeapFile smallHeapFileA = SystemTestUtil.createRandomHeapFile(2, 10, Integer.MAX_VALUE, null, smallHeapFileTuples, "c");		
		...
		for (int i = 0; i < 100000; i++) {
			bigHeapFileTuples.add( smallHeapFileTuples.get( i%10 ) );
		}
		...
		double smallFilter = 1.0;
		filterSelectivities.put("bigTable", 1.0);
		filterSelectivities.put("a", smallFilter);
		filterSelectivities.put("b", smallFilter);
		filterSelectivities.put("c", smallFilter);
		filterSelectivities.put("d", smallFilter);
		filterSelectivities.put("e", smallFilter);
		filterSelectivities.put("f", smallFilter);
		filterSelectivities.put("g", smallFilter);
		filterSelectivities.put("h", smallFilter);
		filterSelectivities.put("i", smallFilter);
		filterSelectivities.put("j", smallFilter);
		filterSelectivities.put("k", smallFilter);
		filterSelectivities.put("l", smallFilter);
		filterSelectivities.put("m", smallFilter);
		filterSelectivities.put("n", smallFilter);
		...
		*/
		nodes.add(new LogicalJoinNode("h", "bigTable", "c0", "c0", Predicate.Op.EQUALS));
		...
		j = new JoinOptimizer(
				...
				p.generateLogicalPlan(tid, "SELECT COUNT(a.c0) FROM bigTable, a, b, c, d, e, f, g, h WHERE bigTable.c0 = h.c0 AND a.c1 = b.c1 AND b.c0 = c.c0 AND c.c1 = d.c1 AND d.c0 = e.c0 AND e.c1 = f.c1 AND f.c0 = g.c0 AND g.c1 = h.c1;"),		
				nodes);
		...
		Assert.assertEquals("bigTable",result.get(result.size()-1).t2Alias);
	}
	
	/**
	 * Test a join ordering with an inequality, to make sure the inequality gets put
	 * as the outermost join
	 */
    ...
    public static junit.framework.Test suite() {
        return new JUnit4TestAdapter(JoinOptimizerTest.class);
    }
}
